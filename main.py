...
def signup(user: model.schemas.UserModel, db: Session = Depends(get_db)):
    return crud.create_user(db, model.schemas.UserModel(email=user.email, name=user.name, hashed_password=user.hashed_password, role=user.role, phone=user.phone))
